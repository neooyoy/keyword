!function () {
    ...
    (function () {
        function t(e) {
            ...
            if ("complete" == s.readyState)e.call(n, "lazy"); else {
        ...
        h.push("/plugins/bootstrap3-dialog/css/prettify.css"), function (e) {
            function t(n) {
                if (n !== i) {
        ...
        var v = function () {
            ...
            return function () {
                ...
                function o(e) {
                    for (var t = void 0, i = e.firstChild; i; i = i.nextSibling)var n = i.nodeType, t = 1 === n ? t ? e : i : 3 === n ? w.test(i.nodeValue) ? e : t : t;
                    return t === e ? void 0 : t
                }
                ...
                c(S, ["default-code"]), c(a([], [["pln", /^[^<?]+/], ["dec", /^<!\w[^>]*(?:>|$)/], ["com", /^<\!--[\S\s]*?(?:--\>|$)/], ["lang-", /^<\?([\S\s]+?)(?:\?>|$)/], ["lang-", /^<%([\S\s]+?)(?:%>|$)/], ["pun", /^(?:<[%?]|[%?]>)/], ["lang-", /^<xmp\b[^>]*>([\S\s]+?)<\/xmp\b[^>]*>/i], ["lang-js", /^<script\b[^>]*>([\S\s]*?)(<\/script\b[^>]*>)/i], ["lang-css", /^<style\b[^>]*>([\S\s]*?)(<\/style\b[^>]*>)/i], ["lang-in.tag", /^(<\/?[a-z][^<>]*>)/i]]), ["default-markup", "htm", "html", "mxml", "xhtml", "xml", "xsl"]), c(a([["pln", /^\s+/, e, " 	\r\n"], ["atv", /^(?:"[^"]*"?|'[^']*'?)/, e, "\"'"]], [["tag", /^^<\/?[a-z](?:[\w-.:]*\w)?|\/?>$/i], ["atn", /^(?!style[\s=]|on)[a-z](?:[\w:-]*\w)?/i], ["lang-uq.val", /^=\s*([^\s"'>]*(?:[^\s"'/>]|\/(?=\s)))/], ["pun", /^[/<->]+/], ["lang-js", /^on\w+\s*=\s*"([^"]+)"/i], ["lang-js", /^on\w+\s*=\s*'([^']+)'/i], ["lang-js", /^on\w+\s*=\s*([^\s"'>]+)/i], ["lang-css", /^style\s*=\s*"([^"]+)"/i], ["lang-css", /^style\s*=\s*'([^']+)'/i], ["lang-css", /^style\s*=\s*([^\s"'>]+)/i]]), ["in.tag"]), c(a([], [["atv", /^[\S\s]+/]]), ["uq.val"]), c(l({
                    ...
                    cStyleComments: !0,
                    types: C
                }), ["c", "cc", "cpp", "cxx", "cyc", "m"]), c(l({keywords: "null,true,false"}), ["json"]), c(l({
                    keywords: v,
                    hashComments: !0,
                    cStyleComments: !0,
                    verbatimStrings: !0,
                    types: C
                }), ["cs"]), c(l({keywords: y, cStyleComments: !0}), ["java"]), c(l({
                    keywords: f,
                    hashComments: !0,
                    multiLineStrings: !0
                }), ["bash", "bsh", "csh", "sh"]), c(l({
                ...
                var _ = p.PR = {
                    ...
                    prettyPrint: t = t = function (t, i) {
                            ...
                            for (var i = p.PR_SHOULD_USE_CONTINUATION ? m.now() + 250 : 1 / 0; l.length > y && i > m.now(); y++) {
                                ...
                                if (u = s.className, (c !== S || b.test(u)) && !L.test(u)) {
                                    ...
                                    if (!f) {
                                        ...
                                        if (x.test(s.tagName))O = 1; else var O = s.currentStyle, E = a.defaultView, O = (O = O ? O.whiteSpace : E && E.getComputedStyle ? E.getComputedStyle(s, e).getPropertyValue("white-space") : 0) && "pre" === O.substring(0, 3);
                        ...
                        for (var s = i || document.body, a = s.ownerDocument || document, s = [s.getElementsByTagName("pre"), s.getElementsByTagName("code"), s.getElementsByTagName("xmp")], l = [], c = 0; s.length > c; ++c)for (var u = 0, f = s[c].length; f > u; ++u)l.push(s[c][u]);
                        var s = e, m = Date;
                        m.now || (m = {
                            now: function () {
                                return +new Date
